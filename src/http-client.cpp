...
static size_t curl_write_function(char *buffer, size_t size, size_t nitems,
				  std::string *response);
...
HttpClientFormField::HttpClientFormField(std::string name, std::string filename,
...
size_t curl_write_function(char *buffer, size_t size, size_t nitems,
			   std::string *response)
{
...
bool HttpClient::send()
{
	...
	if (curl) {
		...
		if (this->method == HTTP_CLIENT_METHOD_GET &&
		...
		if (this->error.curl_code == CURLE_OK &&
		    this->error.http_code == 200 &&
		    this->success_json_item != HTTP_CLIENT_BOOL_ITEM_FALSE &&
		    this->error_json_item.empty())
