...
NudgisStreams::NudgisStreams(obs_output_t *output)
{
	if (output != NULL) {
		...
		this->video_bitrate = (int)obs_data_get_int(vsettings, "bitrate") * 1000;
		this->audio_bitrate = (int)obs_data_get_int(asettings, "bitrate") * 1000;
...
const std::string NudgisStreams::GetJson() const
{
	...
	res << "[{\"width\": " << this->width << ", \"height\": " << this->height
	    << ", \"video_bitrate\": " << this->video_bitrate << ", \"audio_bitrate\": " << this->audio_bitrate
	    << ", \"framerate\": " << framerate << "}]";
...
NudgisData::NudgisData(NudgisConfig *nudgis_config) : NudgisData(NULL, nudgis_config) {}
...
const std::string &NudgisData::GetUrlPrefix()
{
	if (this->url_prefix == NULL) {
		this->url_prefix =
			new std::string(*this->GetServerVersion() < QVersionNumber(8, 2) ? "medias/resource/" : "");
	}
...
const QVersionNumber *NudgisData::GetServerVersion()
{
	...
	if (*result == NULL) {
		...
		bool getdata_result;
		std::string getdata_response =
			this->GetData(this->GetApiBaseUrl(), this->GetApiBaseGetdata(), &getdata_result);
		if (getdata_result) {
	...
	if (config_channel == DEF_CHANNEL) {
		if (this->oid_personal_channel == OID_PERSONAL_CHANNEL_UNDEF) {
			...
			std::string getdata_response = this->GetData(
				this->GetChannelsPersonalUrl(), this->GetChannelsPersonalGetdata(), &getdata_result);
			if (getdata_result) {
				obs_data_t *obs_data = obs_data_create_from_json(getdata_response.c_str());
				if (obs_data != NULL) {
...
const std::string &NudgisData::GetPreparePostdata(const NudgisStreams *nudgis_streams)
{
	...
	prepare_postdata << PARAM_API_KEY << this->nudgis_config->api_key << "&" << PARAM_MULTI_STREAMS
			 << DEF_MULTI_STREAMS << "&" << PARAM_STREAMS << nudgis_streams->GetJson() << "&" << PARAM_TITLE
			 << this->nudgis_config->stream_title << "&" << PARAM_CHANNEL << this->GetStreamChannel();
...
std::list<HttpClientFormField> &NudgisData::GetUploadFormFields(std::string &file_basename, const char *read_buffer,
...
const std::string &NudgisData::GetUploadCompletePostdata(std::string &upload_id, bool check_md5,
...
const std::string &NudgisData::GetMediasAddPostdata(std::string &upload_id, std::string &title)
{
	...
	mediasadd_postdata << PARAM_ORIGIN << ORIGIN << "&" << PARAM_CODE << upload_id << "&" << PARAM_API_KEY
			   << this->nudgis_config->api_key << "&" << PARAM_TITLE << title << "&" << PARAM_CHANNEL
...
const std::string &NudgisData::GetMediasDeletePostdata(bool delete_metadata, bool delete_resources)
{
	...
	medias_delete_postdata << PARAM_API_KEY << this->nudgis_config->api_key << "&" << PARAM_OID << this->oid << "&"
			       << PARAM_DELETE_METADATA << delete_metadata << "&" << PARAM_DELETE_RESOURCES
			       << delete_resources;
...
enum NudgisTestParamsData::NUDGISDATA_LIVE_TEST_RESULT NudgisData::TestLive()
{
	...
	obs_log(LOG_DEBUG, "TestLive result: %s", NUDGISDATA_LIVE_TEST_RESULT_STR[result]);
...
static bool nudgis_initialize(void *data, obs_output_t *output)
{
	...
	std::string prepare_response = nudgis_data->PostData(nudgis_data->GetPrepareUrl(),
...
static obs_properties_t *nudgis_properties(void *data)
{
	...
	p = obs_properties_add_list(ppts, "server", obs_module_text("Server"), OBS_COMBO_TYPE_LIST,
				    OBS_COMBO_FORMAT_STRING);
...
void NudgisUpload::run()
{
	...
	obs_log(LOG_INFO, "enter in nudgis_upload_file with filename: %s (%s)", this->filename, file_basename.c_str());
	...
	if (file.is_open()) {
		...
		uint64_t chunks_count = (uint64_t)ceil((double)total_size * 1.0 / (double)chunk_size);
		...
		if (read_buffer != NULL) {
			...
			if (http_client->getSendSuccess()) {
				...
				while (!file.eof() && !this->canceled && http_client->getSendSuccess()) {
					...
					chunk_index++;
					obs_log(LOG_INFO, "Uploading chunk %" PRIu64 "/%" PRIu64, chunk_index,
						chunks_count);
					if (this->check_md5) {
						// For QT > 6.5
					...
					headers << "Content-Range: bytes " << previous_offset << "-"
						<< current_offset - 1 << "/" << total_size;
					...
					http_client->setFormFields(this->nudgis_data.GetUploadFormFields(
						file_basename, read_buffer, chunk, upload_id));

					http_client->send();
					if (http_client->getSendSuccess()) {
						obs_log(LOG_INFO, "90.0 * current_offset / total_size: %f",
							90.0 * (double)current_offset / (double)total_size);
						emit this->progressUpload(
							(int)(90.0 * (double)current_offset / (double)total_size));
						if (upload_id.length() < 1 &&
						    obs_data_has_user_value(http_client->getResponseObsData(),
									    "upload_id"))
							upload_id = obs_data_get_string(
								http_client->getResponseObsData(), "upload_id");
				...
				if (!this->canceled && http_client->getSendSuccess()) {
					http_client->reset();
					if (this->nudgis_data.PostData(this->nudgis_data.GetUploadCompleteUrl(),
								       this->nudgis_data.GetUploadCompletePostdata(
									       upload_id, this->check_md5, md5sum)) &&
					    !this->canceled) {
						if (this->nudgis_data.PostData(this->nudgis_data.GetMediasAddUrl(),
									       this->nudgis_data.GetMediasAddPostdata(
										       upload_id, file_basename))) {
							if (obs_data_has_user_value(this->nudgis_data.GetHttpClient()
											    .getResponseObsData(),
										    "oid"))
								this->file_uploaded_oid = obs_data_get_string(
									this->nudgis_data.GetHttpClient()
										.getResponseObsData(),
									"oid");
	...
	obs_log(LOG_DEBUG, "  NudgisUpload state: %s", NUDGIS_UPLOAD_STATE_STR[this->state]);
