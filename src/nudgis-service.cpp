...
NudgisStreams::NudgisStreams()
	: NudgisStreams(DEF_WIDTH, DEF_HEIGHT, DEF_VIDEO_BITRATE,
			DEF_AUDIO_BITRATE, DEF_FRAMERATE)
{
}

NudgisStreams::NudgisStreams(int width, int height, int video_bitrate,
...
NudgisStreams::NudgisStreams(obs_output_t *output)
{
	if (output != NULL) {
		...
		this->video_bitrate =
			(int)obs_data_get_int(vsettings, "bitrate") * 1000;
		this->audio_bitrate =
			(int)obs_data_get_int(asettings, "bitrate") * 1000;

		obs_data_release(vsettings);
		obs_data_release(asettings);

		const struct video_output_info *output_video_info =
...
const std::string &NudgisData::GetUrlPrefix()
{
	if (this->url_prefix == NULL) {
		this->url_prefix = new std::string(
			*this->GetServerVersion() < QVersionNumber(8, 2)
				? "medias/resource/"
				: "");
	}
...
const std::string &NudgisData::GetData(const std::string &url,
...
const std::string &NudgisData::PostData(const std::string &url,
...
bool NudgisData::InitFromPrepareResponse(const std::string &prepare_response)
{
	...
	if (obs_data != NULL) {
		obs_data_array_t *streams =
			obs_data_get_array(obs_data, "streams");
		if (streams != NULL) {
			obs_data_t *stream = obs_data_array_item(streams, 0);
			if (stream != NULL) {
				obs_data_set_default_string(
					stream, "server_uri", DEF_SERVER_URI);
				this->server_uri = obs_data_get_string(
					stream, "server_uri");

				obs_data_set_default_string(stream, "stream_id",
							    DEF_STREAM_ID);
				this->stream_id = obs_data_get_string(
					stream, "stream_id");

				obs_data_set_string(settings, "key",
						    this->stream_id.c_str());
...
const QVersionNumber *NudgisData::GetServerVersion()
{
	...
	if (*result == NULL) {
		QVersionNumber version_number =
			QVersionNumber::fromString(DEF_VERSION_NUMBER);
		bool getdata_result;
		std::string getdata_response = this->GetData(
			this->GetApiBaseUrl(), this->GetApiBaseGetdata(),
			&getdata_result);
		if (getdata_result) {
			obs_data_t *obs_data = obs_data_create_from_json(
				getdata_response.c_str());
			if (obs_data != NULL) {
				const char *mediaserver = obs_data_get_string(
					obs_data, "mediaserver");
				if (mediaserver != NULL &&
				    strlen(mediaserver) > 0)
					version_number =
...
const std::string &NudgisData::GetChannel(std::string &config_channel)
{
	...
	if (config_channel == DEF_CHANNEL) {
		if (this->oid_personal_channel == OID_PERSONAL_CHANNEL_UNDEF) {
			bool getdata_result;
			std::string getdata_response = this->GetData(
				this->GetChannelsPersonalUrl(),
				this->GetChannelsPersonalGetdata(),
				&getdata_result);
			if (getdata_result) {
				obs_data_t *obs_data =
					obs_data_create_from_json(
						getdata_response.c_str());
				if (obs_data != NULL) {
					this->oid_personal_channel =
						obs_data_get_string(obs_data,
								    "oid");
					obs_data_release(obs_data);
				}
			}
...
const std::string &
NudgisData::GetPreparePostdata(const NudgisStreams *nudgis_streams)
{
	static std::string result;

	std::ostringstream prepare_postdata;
	prepare_postdata << PARAM_API_KEY << this->nudgis_config->api_key << "&"
			 << PARAM_MULTI_STREAMS << DEF_MULTI_STREAMS << "&"
			 << PARAM_STREAMS << nudgis_streams->GetJson() << "&"
...
NudgisData::GetUploadFormFields(std::string &file_basename,
				const char *read_buffer, size_t chunk,
...
const std::string &
NudgisData::GetUploadCompletePostdata(std::string &upload_id, bool check_md5,
				      QCryptographicHash &md5sum)
{
	static std::string result;

	std::ostringstream uploadcomplete_postdata;
	uploadcomplete_postdata << PARAM_UPLOAD_ID << upload_id << "&"
				<< PARAM_API_KEY << this->nudgis_config->api_key
				<< "&";
	if (check_md5)
		uploadcomplete_postdata
			<< PARAM_MD5 << md5sum.result().toHex().toStdString();
	else
...
const std::string &NudgisData::GetMediasAddPostdata(std::string &upload_id,
						    std::string &title)
{
	static std::string result;

	std::ostringstream mediasadd_postdata;
	mediasadd_postdata << PARAM_ORIGIN << ORIGIN << "&" << PARAM_CODE
			   << upload_id << "&" << PARAM_API_KEY
			   << this->nudgis_config->api_key << "&" << PARAM_TITLE
			   << title << "&" << PARAM_CHANNEL
...
const std::string &NudgisData::GetMediasDeletePostdata(bool delete_metadata,
						       bool delete_resources)
{
	static std::string result;

	std::ostringstream medias_delete_postdata;
	medias_delete_postdata << PARAM_API_KEY << this->nudgis_config->api_key
			       << "&" << PARAM_OID << this->oid << "&"
			       << PARAM_DELETE_METADATA << delete_metadata
...
{
	enum NudgisTestParamsData::NUDGISDATA_LIVE_TEST_RESULT result =
	...
	stream_title << PREFIX_TEST_STREAM_TITLE
		     << QRandomGenerator::global()->bounded(10000, 99999);
	...
	this->PostData(this->GetPrepareUrl(),
		       this->GetPreparePostdata(&nudgis_streams));
	if (this->http_client.getSendSuccess()) {
		this->InitFromPrepareResponse(this->http_client.getResponse());
		this->PostData(this->GetMediasDeleteUrl(),
			       this->GetMediasDeletePostdata(true, true));
	}

	if (this->http_client.getSendSuccess())
		result =
			NudgisTestParamsData::NUDGISDATA_LIVE_TEST_RESULT_SUCESS;
...
static void update_video_keyint_sec(int new_value, obs_output_t *output)
{
	...
	int old_value;
	const char *path =
		obs_frontend_get_current_profile_path(FILENAME_STREAMENCODER);

	settings = obs_data_create_from_json_file_safe(path, "bak");
	old_value = (int)obs_data_get_int(settings, "keyint_sec");
...
static bool nudgis_initialize(void *data, obs_output_t *output)
{
	...
	std::string prepare_response = nudgis_data->PostData(
		nudgis_data->GetPrepareUrl(),
		nudgis_data->GetPreparePostdata(&nudgis_streams), &result);
	if (result) {
		nudgis_data->InitFromPrepareResponse(prepare_response);
		result = nudgis_data->PostData(nudgis_data->GetStartUrl(),
					       nudgis_data->GetStartPostdata());
	}

	return result;
}

const char *nudgis_get_protocol(void *data)
{
...
static obs_properties_t *nudgis_properties(void *data)
{
	...
	obs_property_list_add_string(
		p, obs_module_text(nudgis_config->url.c_str()),
		nudgis_config->url.c_str());
...
struct obs_service_info nudgis_service_info = {
	...
	// The 4 following cb have been deprecated from in 29.1 and replaced by get_connect_info()
	NULL, //const char *(*get_url)(void *data);
	NULL, //const char *(*get_key)(void *data);
	...
	nudgis_get_protocol, //const char *(*get_protocol)(void *data);

	NULL, //const char **(*get_supported_audio_codecs)(void *data);

	nudgis_get_connect_info, //const char *(*get_connect_info)(void *data, uint32_t type);

	NULL, //bool (*can_try_to_connect)(void *data);
};
...
void NudgisUpload::run()
{
	...
	if (file.is_open()) {
		...
		uint64_t chunks_count = (uint64_t)ceil(
			(double)total_size * 1.0 / (double)chunk_size);
		...
		if (read_buffer != NULL) {
			...
			std::string upload_url =
				this->nudgis_data.GetUploadUrl();

			if (http_client->getSendSuccess()) {
				http_client->setUrl(upload_url.c_str());
				http_client->setMethod(
					HttpClient::HTTP_CLIENT_METHOD_POST);

				while (!file.eof() && !this->canceled &&
				       http_client->getSendSuccess()) {
				...
				if (!this->canceled &&
				    http_client->getSendSuccess()) {
					http_client->reset();
					if (this->nudgis_data.PostData(
						    this->nudgis_data
							    .GetUploadCompleteUrl(),
						    this->nudgis_data
							    .GetUploadCompletePostdata(
								    upload_id,
								    this->check_md5,
								    md5sum)) &&
					    !this->canceled) {
						if (this->nudgis_data.PostData(
							    this->nudgis_data
								    .GetMediasAddUrl(),
							    this->nudgis_data.GetMediasAddPostdata(
								    upload_id,
								    file_basename))) {
							if (obs_data_has_user_value(
								    this->nudgis_data
									    ...
									    .getResponseObsData(),
								    "oid"))
								this->file_uploaded_oid = obs_data_get_string(
									this->nudgis_data
										.GetHttpClient()
										.getResponseObsData(),
									"oid");
...
const char *NudgisUpload::GetFileUploadedUrlHtml()
{
	...
	file_uploaded_url_html << "<a href=\"" << file_uploaded_url << "\">"
			       << file_uploaded_url << "</a>";
...
std::string &NudgisUpload::GetFileUploadedUrl()
{
	...
	file_uploaded_url << this->nudgis_data.nudgis_config->url
			  << "/permalink/" << this->GetFileUploadedOid() << "/";
